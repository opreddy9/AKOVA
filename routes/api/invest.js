...
router.get('/getallinvestments/entrepreneur',[auth],async (req,res)=>{
    try{
        // console.log(req.user.id)
        if(req.user.profession !=='entrepreneur'){
            return res.status(400).json({
        ...
        let investments=await investmentask.find({user:req.user.id}).sort({date:-1});
        if(!investments || investments.length==0){
            return res.status(400).json({msg:"No investments are present"})
        }
        res.status(200).json(investments);
...
router.post('/investmentaccepted/:id',[auth,checkObjectId('id')], async (req,res)=>{
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
router.put('/accept/:id',[auth,checkObjectId('id')], async (req,res)=>{
    try {
        if(req.user.profession !=='investor'){
            return res.status(400).json({
                errors:[{msg:"Authorization denied"}]
        ...
        let updatedIdea=await investmentask.findById(req.params.id);
        if (!updatedIdea ) {
            return res.status(404).json({
                msg: "No investment found"
            });
        }
        updatedIdea.status=1;
